...
ofalgorithms %Ist nur für Algorithmen, die mittels \begin{algorithm} umschlossen werden, nötig
...
Beginning with chapter 2 we briefly go over the  history of generative models, introduce large-language-models (LLMs),
...
The goal of this work is to ultimately assist in prompt engineering, some prompt engineering principles were also employed when generating prompt alternatives.
...
However with only this overview we have lost the crucial information of what nodes belong together with multiple new sentences being possible when just following the graph structure, when an edge aggregation is followed by an edge split. To remedy this a select-and-hover function is introduced where we can select nodes then when hovering a node all the selected and hovered nodes relatives from the original data are shown. This allows for an easier overview and and comparability of the nodes, this approach can scale quite well if prompts are sufficiently similar (if all prompts would differ at every position we'd just have a prompt list again).
